...
module SIS
  module Banner
    class Import
      ...
      #  * Course and User must be imported before Enrollment
      IMPORTERS = [:course, :user, :enrollment]
      ...
      def process
        ...
        @files.each do |file|
          if File.file?(file)
            ...
            elsif ['.csv', '.txt'].include? File.extname(file).downcase
        ...
        IMPORTERS.each do |importer|
          @csvs[importer].reject! do |csv|
            begin
              rows = 0
              ::CSV.open(csv[:fullpath], "rb", BannerBaseImporter::PARSE_ARGS) do |faster_csv|
              ...
              add_error(csv, "Malformed input file")
              true
            end
          end
        end
        ...
        if (@parallelism > 1)
          ...
          IMPORTERS.each do |importer|
            importerObject = SIS::Banner.const_get(importer.to_s.camelcase + 'Importer').new(self)
        ...
        if @batch
          error_report = ErrorReport.log_exception(:sis_import, e,
            :message => "Importing input file for account: #{@root_account.id} (#{@root_account.name}) sis_batch_id: #{@batch.id}: #{e.to_s}",
            :during_tests => false
          )
          add_error(nil, "Error while importing Banner file. Please contact support. (Error report #{error_report.id})")
        else
      ...
      def rebalance_csvs(importer)
        ...
        begin
          ...
          @csvs[importer].each do |csv|
            ...
            ::CSV.foreach(csv[:fullpath], BannerBaseImporter::PARSE_ARGS) do |row|
      ...
      def process_file(base, file)
        ...
        if File.file?(csv[:fullpath]) && ['.csv', '.txt'].include?(File.extname(csv[:fullpath]).downcase)
          # validate UTF-8
          begin
          ...
          begin
            ::CSV.foreach(csv[:fullpath], BannerBaseImporter::PARSE_ARGS.merge(:headers => false)) do |row|
              row.each(&:downcase!)
              importer = IMPORTERS.index do |importer|
                if SIS::Banner.const_get(importer.to_s.camelcase + 'Importer').send('is_' + importer.to_s + '_csv?', row)
                  @csvs[importer] << csv
              ...
              add_error(csv, "Couldn't find Banner file import headers") if importer.nil?
              break
            end
          rescue ::CSV::MalformedCSVError
            add_error(csv, "Malformed input file")
