...
module Api::V1::DiscussionTopics
  ...
  def discussion_topic_api_json(topic, context, user, session, include_assignment = true)
    ...
    api_json(topic, user, session, {
                  :only => %w(id title assignment_id delayed_post_at lock_at last_reply_at posted_at root_topic_id podcast_has_student_posts grade_replies_separately reply_assignment_id),
      ...
      if include_assignment && topic.reply_assignment
        extend Api::V1::Assignment
        json[:reply_assignment] = assignment_json(topic.reply_assignment, user, session, include_discussion_topic: false)
      end
      json
    end
  end
