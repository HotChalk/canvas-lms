  ...
  def index
    if authorized_action(@context, @current_user, :read)
      ...
      if @context.feature_enabled?(:differentiated_assignments) && includes.include?('items')
        ...
        opts[:discussion_visibilities] = DiscussionTopic.visible_to_users_in_course_with_da(user_ids, @context.id).pluck(:id)
        opts[:quiz_visibilities] = Quizzes::Quiz.visible_to_users_in_course_with_da(user_ids,@context.id).pluck(:quiz_id)
      end
