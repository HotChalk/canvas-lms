(function() {
  CKEDITOR.plugins.add('instructure_links', {
    icons: 'instructure_links',
    hidpi: true,
    init: function(editor) {
      ...
      function updateLinks(arg) {
        ...
        if(arg == true && updateLinks.counter != 0) {
          ...
          $(editor.document.getBody().$).find("a").each(function() {
            var $link = $(this);
        ...
        $(editor.document.getBody().$).find("iframe").each(function() {
          var $frame = $(this);
          var $link = $("<img/>");
          ...
          if(!width || width == '100%' || width == 'auto') {
            var edWidth = $(editor.container.$).width();
        ...
        .find(".iframe_placeholder").each(function() {
          var edWidth = $(editor.container.$).width();
          var $holder = $(this);
          if($(editor.container.$).hasScrollbar() || true) {
            edWidth -= $(editor.container.$).scrollbarWidth();
          }
      ...
      editor.addCommand('instructureLinks', {
        exec: function(api) {
          var $editor = $("#" + api.element.getId());
          // TODO bookmarking
          /*
          ...
          */
          ...
          var e = api.getSelection().getSelectedElement();
          while(e && e.nodeName != 'A' && e.nodeName != 'BODY' && e.parentNode) {
            e = e.parentNode;
          }
          var $a = (e && e.nodeName == 'A' ? $(e) : null);
          if($a) {
      ...
      editor.ui.addButton('instructure_links', {
        label: 'Link to URL',
        command: 'instructureLinks',
        toolbar: 'insert'
      });
