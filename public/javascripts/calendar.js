...
define([
    ...
    'jqueryui/tabs' /* /\.tabs/ */
], function (INST, I18n, $, userSettings, calendarMonths, _) {
    ...
    function eventIntersect(event, ui) {
        ...
        var $week = null;
        var $day = null;
        if (!$(".calendar_day_undated").data('drag_position')) {
            return;
        }
        if ($lastIntersection) {
    ...
    var eventDraggable = {
        ...
        drag: function (event, ui) {
            ...
            var $day = eventIntersect(event, ui);
            if ($day && $day.hasClass('drop_target')) {
                return;
            }
            $(".drop_target").removeClass('drop_target');
            if ($day) {
    ...
    function updateEvent(data, $event, batch) {
        ...
        if (!event.start_at && !$event && !calendar.showingUndatedEvents) {
            var isCalendarChecked = $('#group_' + event.context_code + ':checked').length > 0, existsEvent;
            calendar.hiddenUndatedEvents = calendar.hiddenUndatedEvents || [];
            existsEvent = _.find(calendar.hiddenUndatedEvents, function(ev) {
                return ev[Object.keys(ev)[0]].id === event.id;
            });
            if (_.isUndefined(existsEvent) && isCalendarChecked) {
        ...
        $box.dialog({
            ...
            open: function () {
                ...
                $("#edit_event .context_select").each(function () {
                    ...
                    if (idsToShow.length) {
                        $select.val(forceValue || idsToShow[0])
                    }
                    ;
                    $(this).showIf(isNew);
                ...
                if (data.event_type == "assignment") {
                    ...
                    if ($("#edit_assignment_form select.context_id option").filter(function () {
    ...
    $(document).ready(function () {
        ...
        $("#edit_calendar_event_form .time_field").time_field()
            .blur(function (event) {
                ...
                if ($("#edit_calendar_event_form .time_field.start_time").next(".datetime_suggest").hasClass('invalid_datetime')) {
                    start_time = null;
                }
                ...
                if ($("#edit_calendar_event_form .time_field.end_time").next(".datetime_suggest").hasClass('invalid_datetime')) {
                    end_time = null;
                }
        ...
        $(".group_reference_checkbox").bind('change click',function () {
            ...
            setTimeout(function () {
                ...
                if ($(item).attr('checked')) {
                    ...
                    var context_code = $(item).attr('id').replace("group_", ""),
                        $undated_count = $(".show_undated_link .undated_count");
                    calendar.hiddenUndatedEvents = _.reject(calendar.hiddenUndatedEvents, function (event) {
                        return event[Object.keys(event)[0]].context_code == context_code;
                    });
                    $undated_count.text(calendar.hiddenUndatedEvents.length)
                }
                logCheckedContexts();
        ...
        $(".calendar_container").delegate('.calendar_day', 'mouseover',function (event) {
            ...
            if ($container.data('prevent_hover')) {
                return;
            }
